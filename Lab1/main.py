...
# Form implementation generated from reading ui file 'D:\Универ\OKS\Lab1\UI.ui'
#
...
class Ui_PortApp(object):
    def __init__(self, mainWindow):
        ...
        self.port.readyRead.connect(self.onRevieveBytes)
...
"font-weight: bold;")
        ...
        # заголовок для выбора номера порта
        ...
        # выпадающий список с количеством бит в байте
        ...
        # поле вывода количества переданных байт
    ...
    def onSendBytes(self):
        ...
        sendedBytesCount: int = len(text) - 1
    ...
    def onRevieveBytes(self):
        text = self.port.readAll().data()
        if len(text) == 0:
            QMessageBox.warning(None, "Error", "Can't read data from port")
            sys.exit(app.exec_())
        else:
...
#         if event.type() ==QtCore.QtEvent.KeyPress and obj is self.inputTextEdit:
#             if event.key() == QtCore.Qt.Key_Return and self.inputTextEdit.hasFocus():
#                 self.onSendBytes()
