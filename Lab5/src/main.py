...
class Ui_PortApp(object):
    ...
    def setupUi(self, PortApp):
        ...
        PortApp.resize(700, 650)
        PortApp.setMinimumSize(QtCore.QSize(700, 650))
        PortApp.setMaximumSize(QtCore.QSize(700, 650))
        ...
        self.baudRateValueLabel.setGeometry(QtCore.QRect(467, 390, 50, 21))
        ...
        self.sentBytesLabel.setGeometry(QtCore.QRect(370, 420, 101, 21))
        ...
        self.sentBytesValueLabel.setGeometry(QtCore.QRect(473, 420, 50, 21))
        ...
        self.sendedBytesTextEdit = QtWidgets.QTextEdit(self.centralwidget)
        self.sendedBytesTextEdit.setEnabled(True)
        self.sendedBytesTextEdit.setGeometry(QtCore.QRect(370, 460, 300, 131))
        self.sendedBytesTextEdit.setReadOnly(True)
        self.sendedBytesTextEdit.setObjectName("sendedBytesTextEdit")
    ...
    def onSendBytes(self):      
        ...
        cadres: list[str] = bitstaffing.split_sended_on_cadres(input_data)
        data_to_send: str = ""
        ...
        for cadre in cadres:
            ...
            data: str = bitstaffing.get_data(cadre)
    ...
    def onRecieveBytes(self):
        recieved_data: str = self.port.readAll().data().decode()
        if len(recieved_data) == 0:
            QMessageBox.warning(None, "Error", "Data cannot be read")
            sys.exit(app.exec_())
        else:
            cadres: list[str] = bitstaffing.split_recieved_on_cadres(recieved_data)
            data_to_output: str = ""
            highlighted_data: str = ""
            for cadre in cadres:
                highlighted_data += bitstaffing.get_highlighted_data(cadre)

                destaffed_data = bitstaffing.de_bit_staffing(cadre[8::1])
                recieved_fcs: str = destaffed_data[-2::1]
                data: str = destaffed_data[8:-2:1]          # извлечь data
                
                calculated_fcs: str = hemming.get_fcs(data)
                if recieved_fcs != calculated_fcs:
                    data = hemming.fix(data, recieved_fcs)
